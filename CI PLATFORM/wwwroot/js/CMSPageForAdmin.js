...
function deleteChkCms() {
    var checkboxes = document.getElementsByClassName('checkbox-cms');
    var checkedValues = [];
    ...
    if (checkedValues.length >0) {
        ...
        $.ajax({
            url: "/Admin/RemoveSelectedCms",
            type: "Put",
            data: {
                checkedCmsValues: checkedValues
            },
            success: function (response) {
                getCMS();
                $('.userdetails').html($(response).find('.userdetails').html());
