...
namespace ciplatform.repository.Repository
{
    public class AdminRepository : IAdminInterface
    {
        ...
        public IPagedList<StoryPageForAdminViewModel> getStories(string searchkeyword, int pageIndex)
        {
            // status = 2 means status is pending, 1 means accepted 3 means rejected
            var Storydata = _CidbContext.Stories.Include(m => m.Mission).Include(m => m.User).Where(m => (searchkeyword == null || m.Title.Contains(searchkeyword) || m.Mission.Title.Contains(searchkeyword) || m.User.FirstName.Contains(searchkeyword) || m.User.SecondName.Contains(searchkeyword)) && m.DeletedAt == null).ToList();
            //&& m.Status == 2
            ...
            return StoryDetails.ToPagedList(pageIndex, 9);
